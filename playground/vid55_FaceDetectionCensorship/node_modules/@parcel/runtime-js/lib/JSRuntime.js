...
var _default = new _plugin.Runtime({
  apply({
    ...
    for (let _ref of bundleGraph.getBundleGroupsReferencedByBundle(bundle)) {
      let {
        bundleGroup,
        dependency
      } = _ref;
      ...
      let bundles = bundleGraph.getBundlesInBundleGroup(bundleGroup).sort(bundle => bundle.getEntryAssets().map(asset => asset.id).includes(bundleGroup.entryAssetId) ? 1 : -1); // CommonJS is a synchronous module system, so there is no need to load bundles in parallel.
      ...
      let loaderModules = bundles.map(b => {
        ...
        let url = (0, _utils.urlJoin)((0, _nullthrows.default)(b.target.publicUrl), (0, _nullthrows.default)(b.name));
      ...
      if (loaderModules.length > 0) {
        ...
        if (loaderModules.length > 1 && (bundle.env.outputFormat === 'global' || !bundles.every(b => b.type === 'js'))) {
        ...
        for (let bundle of bundles) {
          let filePath = (0, _nullthrows.default)(bundle.getMainEntry()).filePath;
          ...
          if (bundle.target.publicUrl == null) {
            throw new Error('JSRuntime: Bundle target did not have a publicUrl');
          }

          assets.push({
            filePath: filePath + '.js',
            code: `module.exports = '${(0, _utils.urlJoin)(bundle.target.publicUrl, (0, _nullthrows.default)(bundle.name))}'`,
            dependency
          });
        }
      }
