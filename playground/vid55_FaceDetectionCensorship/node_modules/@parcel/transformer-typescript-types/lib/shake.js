...
function shake(ts, moduleGraph, context, sourceFile) {
  ...
  let visit = node => {
    ...
    if (ts.isVariableStatement(node)) {
      ...
      node.modifiers = (node.modifiers || []).filter(m => m.kind !== ts.SyntaxKind.ExportKeyword); // Add export modifier if all declarations are exported.
    ...
    if (ts.isIdentifier(node) && currentModule.names.has(node.text)) {
      return ts.createIdentifier((0, _nullthrows.default)(currentModule.names.get(node.text)));
...
function generateImports(ts, moduleGraph) {
  ...
  for (let [specifier, names] of moduleGraph.getAllImports()) {
    ...
    if (defaultSpecifier || namedSpecifiers.length > 0) {
      let importClause = ts.createImportClause(defaultSpecifier, ts.createNamedImports(namedSpecifiers));
      importStatements.push(ts.createImportDeclaration(undefined, undefined, importClause, // $FlowFixMe
      ts.createLiteral(specifier)));
    }
  }
