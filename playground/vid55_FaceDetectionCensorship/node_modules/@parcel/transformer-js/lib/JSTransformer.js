...
var _env = _interopRequireDefault(require("./visitors/env"));
...
var _default = new _plugin.Transformer({
  ...
  async parse({
    ...
    if (!options.scopeHoist && !canHaveDependencies(code) && !ENV_RE.test(code) && !FS_RE.test(code)) {
      return null;
    }
  ...
  async transform({
    ...
    let ast = asset.ast;
    let code = await asset.getCode(); // Inline environment variables

    if (!asset.env.isNode() && (ast.isDirty || ENV_RE.test(code))) {
      walk.simple(ast.program, _env.default, {
        asset,
        env: options.env
      });
    ...
    if (!asset.env.isNode()) {
      ...
      if (fsDep && FS_RE.test(code)) {
        ...
        if (!ignore) {
          (0, _traverse.default)(ast.program, _fs.default, null, asset);
        }
      } // Insert node globals
