...
const bufferTemplate = (0, _template.default)('Buffer(CONTENT, ENC)');
var _default = {
    ...
    if (referencesImport(path, 'fs', 'readFileSync')) {
      ...
      try {
        ...
        if (err instanceof NodeNotEvaluatedError) {
          ...
          err.fileName = asset.filePath; // asset.generateErrorMessage(err);
...
function evaluate(path, vars) {
  ...
  if (path.isCallExpression() && referencesImport(path, 'path', 'join') && path.node.arguments.every(n => t.isStringLiteral(n))) {
    // e.g. path.join("literal", "another_literal")
    return _path.default.join(...path.node.arguments.map(n => n.value));
  } else {
